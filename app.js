...
const Modules = [
  ...
  ["Repair Memory","Saved fixes","repair","🧠"],["Suppliers","FleetPride etc.","suppliers","◆"],["AI Assistant","Master command","ai","RW"],["Reports","Income + labor","reports","▥"],["Settings","Themes / debug","settings","⚙"],["Emergency","Roadside flow","faults","⚠"]
];
...
const Actions = {
  run(action, el){
    ...
    if(action==="open-ai-drawer") AiDrawer.open();
    if(action==="parts-ai-search") PartsBrain.run();
    if(action==="parts-upload-photo"){ AiDrawer.open(); $("aiUploadPhoto")?.click(); }
    if(action==="parts-take-photo"){ AiDrawer.open(); $("aiTakePhoto")?.click(); }
...
const GpsModule = {
  render(){
    ...
    if(!panel) return;
    panel.innerHTML = `<div class="pro-panel">
      <div class="pro-header"><h3>DOT / GPS COMMAND</h3><span class="status-pill">LOCATION READY</span></div>
      <div class="gps-grid">
        <button class="gps-card" onclick="GpsModule.near('FleetPride near me')"><b>FleetPride</b><span>Find nearest heavy-duty parts counter.</span></button>
        <button class="gps-card" onclick="GpsModule.near('heavy duty truck parts near me')"><b>HD Parts</b><span>Search truck parts suppliers around you.</span></button>
        <button class="gps-card" onclick="GpsModule.near('Cummins dealer near me')"><b>Cummins</b><span>Find Cummins service/parts location.</span></button>
        <button class="gps-card" onclick="GpsModule.near('truck repair near me')"><b>Roadside</b><span>Repair support / roadside location search.</span></button>
        <button class="gps-card gps-wide" onclick="GpsModule.whereAmI()"><b>Where Am I?</b><span>Use phone location permission for coordinates and map link.</span></button>
      </div>
      <div class="view-area" id="gpsOutput">Choose a location tool.</div>
    </div>`;
  },
  near(q){ window.open(`https://www.google.com/maps/search/${encodeURIComponent(q)}`,"_blank"); },
  whereAmI(){
    const out=$("gpsOutput");
    if(!navigator.geolocation){ if(out) out.textContent="Geolocation not supported."; return; }
    if(out) out.textContent="Getting location...";
    navigator.geolocation.getCurrentPosition(pos=>{
      const {latitude,longitude}=pos.coords;
      if(out) out.innerHTML=`Lat: ${latitude.toFixed(6)}<br>Lng: ${longitude.toFixed(6)}<br><br><button class="pro-btn-gold" onclick="window.open('https://maps.google.com/?q=${latitude},${longitude}','_blank')">OPEN MAP</button>`;
