...
const RWDModuleActions = {
  ...
  partsSearch(mode){
    ...
    $("partsResultArea").innerHTML = `<b>${mode.toUpperCase()} READY</b><br>Query: ${escapeHtml(q)}<br>Active VIN: ${escapeHtml(TruckManager.get().vin || "NONE")}<br><br>Backend supplier/OEM verification plugs in here.`;
  },
  openSupplierSearch(){
...
const InvoiceBuilder = {
  ...
  createFromJob(jobId){
    ...
    const labor=Number(s.dollars), partsTotal=PartsManager.getPartsTotal(jobId), subtotal=labor+partsTotal, tax=subtotal*(Number(settings.tax||0)/100), total=subtotal+tax;
    this.active={jobId, settings, truck, summary:s, parts, subtotal, tax, total, status:"DRAFT", created:new Date().toLocaleString()};
  ...
  render(){
    ...
    const lines=[`<tr><td>Labor - ${escapeHtml(s.name)}</td><td>${s.decimalHours}</td><td>${fmtMoney(s.rate)}/hr</td><td>${fmtMoney(s.dollars)}</td></tr>`];
...
function boot(){
  ...
  $("masterCommandInput").addEventListener("keydown", e => { if(e.key==="Enter"){ RwdAI.process(e.target.value); e.target.value=""; }});
