...
const PartsBrain = {
  ...
  setMode(mode){
    ...
    const labels = {
      unknown:"Upload/take a photo or describe the part. I’ll identify likely part type, where used, and what info is needed.",
      number:"Enter OEM or aftermarket number. I’ll explain what it fits, supersessions, cross refs, and estimated cost.",
      cross:"Enter a part number to cross reference OEM ↔ aftermarket.",
      fitment:"Enter year/make/model/engine + part. I’ll show likely options first, then ask last 8 VIN for exact verification."
    };
    if(out) out.innerHTML = `<b>${mode.toUpperCase()} MODE</b><br>${labels[mode] || labels.unknown}`;
  },
  answer(q){
    q=(q||"").trim();
    const low=q.toLowerCase();
    if(!q) return "Ask for a part, upload a photo, scan a number, or describe the truck/engine/job.";
    if(low.includes("water pump") && (low.includes("x15") || low.includes("cummins"))){
      return `<b>2019 International LT/ProStar Cummins X15 — Water Pump Lookup</b><br><br><b>Likely options for this year/engine family:</b><br>• X15 highway-platform water pump family options<br>• OEM and aftermarket choices depend on CPL / supersession<br>• Estimated cost only — vendor/location/core can change price<br><br>${JobKitsManager.render("water pump")}<br><b>Exact fitment step:</b> Send last 8 VIN to verify final part number.`;
    }
    if(low.match(/\\b\\d{5,}\\b/) || low.includes("part number") || low.includes("what does") || low.includes("fit")){
      return `<b>Part Number Lookup / Cross Reference</b><br>Query: ${escapeHtml(q)}<br><br>Result should show:<br>• What this part fits<br>• OEM number / aftermarket cross refs<br>• Supersessions<br>• Estimated market cost range<br>• Related seals, tools, fluids, and job kit<br>• Last 8 VIN prompt when exact fitment matters<br><br>Backend supplier/OEM search plugs in next.`;
    }
    return `<b>Parts Lookup Brain</b><br>Query: ${escapeHtml(q)}<br><br>I will show likely OEM / aftermarket options first, estimated cost range, job kit items, and ask last 8 VIN only when exact fitment matters.<br><br>${JobKitsManager.render(q)}`;
  },
  run(){
